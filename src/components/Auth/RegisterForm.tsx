...
export const RegisterForm: React.FC<RegisterFormProps> = ({ onSwitchToLogin }) => {
  ...
  const validationError = useMemo(() => {
    ...
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return 'Invalid email format';
    ...
    <div className="min-h-screen flex items-center justify-center bg-spotify-black p-4 transition-colors duration-300">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-app-text transition-colors">Sign up for Spotify</h1>
        </div>
        ...
        <div className="mt-8 pt-8 border-t border-black/10 dark:border-white/10 text-center">
          <p className="text-spotify-gray text-sm">
            Already have an account? <span onClick={onSwitchToLogin} className="text-app-text hover:text-spotify-green cursor-pointer underline font-bold transition-colors">Log in</span>
          </p>
        </div>
      </motion.div>
    </div>
