...
export const YoutubeImport = () => {
  ...
  const [previewItem, setPreviewItem] = useState<YoutubeSearchItem | null>(null);
  ...
  const handleOpenPreview = (item: YoutubeSearchItem) => {
    setIsPlaying(false); // Pause internal music
    setPreviewItem(item);
  };
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl md:text-3xl font-bold tracking-tight text-app-text">Import from YouTube</h1>
                <p className="text-spotify-gray text-xs md:text-sm">Convert and add videos to your library</p>
              </div>
            </div>
            <button 
              onClick={() => setCurrentView('home')}
              className="text-spotify-gray hover:text-app-text transition-colors flex items-center gap-2 text-xs md:text-sm font-medium"
            >
          ...
          <div className="mb-6 md:mb-10">
            <div className="relative group">
              <Search className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${isDebouncing || loading ? 'text-spotify-green' : 'text-spotify-gray group-focus-within:text-app-text'}`} size={20} />
              <input
                type="text"
                placeholder="Search or paste YouTube URL..."
                value={keyword}
                onChange={(e) => setKeyword(e.target.value)}
                className="w-full bg-spotify-light border border-black/5 dark:border-white/5 rounded-full py-3 md:py-4 pl-12 pr-12 text-sm md:text-lg focus:ring-2 focus:ring-black/10 dark:focus:ring-white/10 outline-none transition-all shadow-xl text-app-text"
              />
